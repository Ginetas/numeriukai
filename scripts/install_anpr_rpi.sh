...
install_gstreamer_and_coral() {
  log "Installing GStreamer plugins and Coral EdgeTPU runtime..."
  ...
  if ! dpkg -l | grep -q libedgetpu1-std; then
    ...
    log "Coral EdgeTPU runtime already present."
  fi
}
...
main() {
  ...
  compose_up
  create_systemd_service
  log "Installation complete. Access frontend at http://<RPi-IP>:3000 and backend health at http://<RPi-IP>:8000/healthz"
}
