...
## Frontend
- Next.js (App Router) su TypeScript, TailwindCSS, React Query ir Zustand.
- Sidebar nuorodos į dashboard, konfigūracijos puslapius ir integracijas.
- `/dashboard` puslapis jungiasi prie `/healthz`, rodo statistiką ir live įvykių lentelę (WebSocket + polling fallback).
- `/config/*` puslapiai leidžia kurti/redaguoti/ištrinti kameras, zonas, modelius, jutiklius.
- `/integrations` puslapis tvarko eksporterius ir leidžia paleisti testinius requestus.
...
- Greitam patikrinimui prieš commit galite naudoti `scripts/precommit-check.sh`, kuris ieško konfliktų žymių, CRLF eilučių pabaigų ir validuoja `docker-compose` sintaksę.
